...
public class HtmlGeneratorTool implements Tool<String> {
    ...
    @Override
    public String apply(CompositeShape compositeShape) {
        return compositeShape.getShapeList().stream().reduce("", (totalHtml, shape) -> totalHtml + this.apply(shape), String::concat);
    }
